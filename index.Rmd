...
> <span style="color:red">Let's add another column that tells the season in which the project was launched.</span>
...
> <span style="color:red">Let's add a column for duration of the project (in days).</span>
...
####Predictions vs Actual Project Outcomes (First 50 predictions)
```{r}
head(prediction, 50) %>% kable() %>%
  kable_styling() %>% scroll_box()
```
